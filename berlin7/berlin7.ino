...
void setup() {
  ...
  // initialize Timer2, so keep PWM on 9 alive
...
void loop() {
  ...
  if ( counter >= chaosNumber ){
    reflip();
    counter = 0;
    if ( (recycleNum)%30 == 0 ) writeRecycle2();
...
void writeRecycle(){
  ...
  //former recycle page --- replaced by writeRecycle2()
...
void emptyBuffer(){
  ...
  nextAddress = myStrCopy( 0,"                " , screen1[0], 16); //to be changed of course :-)
  nextAddress = myStrCopy( 0,"                " , screen1[1], 16); //to be changed of course :-)
  nextAddress = myStrCopy( 0,"                " , screen1[2], 16); //to be changed of course :-)
  nextAddress = myStrCopy( 0,"                " , screen1[3], 16); //to be changed of course :-)
  bufferToScreen();
...
/**************************************
 *
 * function  patches making the bigger fonts for 2 words: BEAM and PATCHES
 *
 * 
 *
 ***************************************/
...
void patches(){
  ...
  //can be saved by using lcd.print(char(32));
  /*
  byte b0[8] = {//all white
   0b00000,
   0b00000,
...
void writeCharForBigWords(int i, char *textBit){
  if ( textBit[i] == '1' ) { // black can this be saved by using some special char()???? I cannot find an all black
    lcd.write(1);
  }
  else if ( textBit[i] == '0' )  {  //white can be saved by using the space
    lcd.print (char(32)); //lcd.write(2);
  }
  else if ( textBit[i] == '2' )  {  //half down
    lcd.write(4);
  }
  else if ( textBit[i] == '3' )  {  //half up
    lcd.write(3);
  }
  else if ( textBit[i] == '4' )  {  
    lcd.write(5);
...
void snow(unsigned char textNum){
  ...
  if ( textNum == 0 )
    // free lines: 1,2,3, 6, 9,10,11,13,14,15
  {
    setCursorFor16_4(1,1);
    lcd.print("snow snow");
    delay(500);
    setCursorFor16_4(3,5);
    lcd.print("cute cute");
...
void writeRecycle2(){
  ...
  char R[4][5] = { // word "BEAM" which should be changed: first screen shown once (fits on one screen)
    "1310", 
    "1210", 
    "1140", 
    "1510"                      };
  ...
  for ( int j = 0; j < 4; j++ )
  {
    ...
    {
      setCursorFor16_4(j,i);
      writeCharForBigWords(i, R[j] );
    }
  }

  // write the rest in normal font

  setCursorFor16_4(2,3);
  lcd.print("ecycle me");
  delay(500);
  setCursorFor16_4(1,6);
  lcd.print("PUB!    ");
  delay(500);
  setCursorFor16_4(1,6);
  lcd.print("    PUB!");
  ...
  delay(500);
  for ( int i = 0; i < 4; i++ ){
    setCursorFor16_4(i,15);
    lcd.print(char(247));
  }

  for ( int i = 4; i < 16; i++ ){
    setCursorFor16_4(0,i);
    lcd.print(char(245));
  }

  for ( int i = 4; i < 16; i++ ){
    setCursorFor16_4(3,i);
    lcd.print(char(197));
  }
